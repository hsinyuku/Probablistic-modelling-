...
data_BadenW <-  read.csv("data/Germany_cases_deaths_confirmed.csv", header = T) %>%
  mutate(date = ymd(gsub(" 00:00:00", "", Meldedatum))) %>%
  filter(grepl("Baden.", Bundesland), date >= day_data, date <= day_max) %>% 
...
p_underreport_cases <- 19445 / 30761 # numerator: in Table 2, Supplementary
                                     # denomenator: in Table 1, KRI Report
# ----------------------------------------------------------------------------#
